...
contract DeployTestnetCutoverForkTest is Test {
    ...
    /// @notice The CURRENT live (2026-08-06 ERC-2771 cutover) TokenNetwork on Base Sepolia
    ///         (packages/contracts/deployments/base-sepolia.md). Still the pre-ADR-0059 contract:
    ///         it carries `channelCounter` and has no `channelEpoch`, which is exactly what
    ///         testFork_Cutover_LiveDeploymentStillCarriesTheGlobalCounter reads off the fork.
