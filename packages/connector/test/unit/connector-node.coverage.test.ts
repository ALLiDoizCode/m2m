...
describe('ConnectorNode branch coverage', () => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // _createTransportProvider — exhaustiveness guard & managed factory branches
  // ═══════════════════════════════════════════════════════════════════════════
  describe('_createTransportProvider branches', () => {
    it('throws exhaustiveness error for unsupported transport type', async () => {
      // Bypass TypeScript by passing an invalid config shape
      const badConfig = createTestConfig({
        transport: { type: 'unsupported' } as any,
      });
      (ConfigLoader.loadConfig as jest.Mock).mockReturnValue(badConfig);

      const node = new ConnectorNode(testConfigPath, mockLogger);
      await expect(node.start()).rejects.toThrow('Unsupported transport type');
    });

    it('throws when externalUrl is auto without managed=true', async () => {
      const badConfig = createTestConfig({
        transport: {
          type: 'socks5',
      ...
      const node = new ConnectorNode(testConfigPath, mockLogger);
      await expect(node.start()).rejects.toThrow('auto" requires managed: true');
    });

    it('throws when managed auto externalUrl is used without hiddenServiceDir', async () => {
      const badConfig = createTestConfig({
        transport: {
          type: 'socks5',
          socksProxy: 'socks5h://127.0.0.1:9050',
      ...
      const node = new ConnectorNode(testConfigPath, mockLogger);
      await expect(node.start()).rejects.toThrow('managedOptions.hiddenServiceDir');
    });

    it('managed anon factory uses cachedFactory when prewarm succeeds', async () => {
      const factoryFn = jest.fn().mockReturnValue({ mockSdk: true });
      ...
      // The factory should have been cached and used
      expect(createDefaultAnonFactoryMock).toHaveBeenCalled();

      // After start, stop cleanly
      await connectorNode.stop();
    });

    it('managed anon factory re-throws MODULE_NOT_FOUND prewarmError', async () => {
      const modNotFound = Object.assign(new Error('Module not found'), {
        code: 'MODULE_NOT_FOUND',
      });
      const createDefaultAnonFactoryMock = jest.requireMock('../../src/transport')
        .createDefaultAnonFactory as jest.Mock;
      createDefaultAnonFactoryMock.mockRejectedValue(modNotFound);
      ...
      // ManagedAnonClient.start() will be called, which may call the factory.
      // We just verify start completes without throwing from the constructor.
      await connectorNode.start();
      await connectorNode.stop();
    });

    it('managed auto URL resolver throws when deadline exceeded', async () => {
      const createDefaultAnonFactoryMock = jest.requireMock('../../src/transport')
        .createDefaultAnonFactory as jest.Mock;
      createDefaultAnonFactoryMock.mockRejectedValue(
        Object.assign(new Error('mocked'), { code: 'MOCKED_TEST' })
      );
      ...
      // The resolver runs inside SocksTransportProvider.start(), which is mocked
      // to resolve. We need to make socksStartSpy actually invoke the resolver.
      transportSpies.socksStartSpy.mockImplementation(async function (this: {
        options: { resolveExternalUrlOnStart?: () => Promise<string> };
      }) {
        if (this.options.resolveExternalUrlOnStart) {
          await this.options.resolveExternalUrlOnStart();
        }
      });

      await expect(connectorNode.start()).rejects.toThrow('did not become readable within 200ms');
    });
  });
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // _createTransportProvider — managed externalUrl 'auto' sets pending.invalid
  // ═══════════════════════════════════════════════════════════════════════════
  describe('_createTransportProvider managed auto placeholder', () => {
    it('uses pending.invalid placeholder when externalUrl is auto', async () => {
      const createDefaultAnonFactoryMock = jest.requireMock('../../src/transport')
        .createDefaultAnonFactory as jest.Mock;
      createDefaultAnonFactoryMock.mockRejectedValue(
        Object.assign(new Error('mocked'), { code: 'MOCKED_TEST' })
      ...
      // Access the private method directly to inspect the constructed provider
      const provider = (connectorNode as any)._createTransportProvider(managedConfig.transport);

      // The SocksTransportProvider mock stores options; we can inspect it
      expect(provider.options.externalUrl).toBe('wss://pending.invalid/btp');
    });
  });
});
