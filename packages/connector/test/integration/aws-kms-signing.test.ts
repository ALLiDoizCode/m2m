...
try {
  ...
  if (
    ...
    process.env.AWS_KMS_XRP_KEY_ID &&
...
describeIf(awsKmsAvailable)('AWS KMS Signing Integration Tests', () => {
  ...
  beforeAll(() => {
    ...
    const config: KeyManagerConfig = {
      ...
      aws: {
        ...
        xrpKeyId: process.env.AWS_KMS_XRP_KEY_ID!,
        credentials: {
  ...
  describe('XRP claim signing with real AWS KMS key', () => {
    it('should sign XRP claim with real AWS KMS key', async () => {
      // Create test claim message (simplified - doesn't use ripple-binary-codec)
      const channelId = '0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF0123456789ABCDEF';
      const amount = '1000000'; // 1 XRP in drops
      const testMessage = Buffer.from(`${channelId}:${amount}`);

      const xrpKeyId = process.env.AWS_KMS_XRP_KEY_ID!;

      // Sign message using AWS KMS
      const signature = await keyManager.sign(testMessage, xrpKeyId);
      ...
      // Get public key
      const publicKey = await keyManager.getPublicKey(xrpKeyId);

      expect(publicKey).toBeDefined();
      expect(publicKey.length).toBe(32); // ed25519 public keys are 32 bytes

      // Note: Full signature verification would require:
      // 1. Using ripple-binary-codec to encode claim properly
      // 2. Using ed25519 verification (via ripple-keypairs or similar)
      // This is beyond the scope of this test - the test verifies that signing works
    }, 30000); // 30 second timeout for AWS API calls
  });
