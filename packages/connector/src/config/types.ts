...
export interface PeerConfig {
  ...
  /**
   * Per-peer transport. Only direct TCP is supported (retained for forward
   * compatibility and config-shape stability).
   */
  transport?: 'direct';
 ...
 *
 * Only direct TCP transport is supported. The SOCKS5 / ATOR overlay transport
 * was removed; `type` is retained as a single-member discriminant so existing
 * `transport: { type: 'direct' }` configs continue to load unchanged.
 */
export type TransportConfig = { type: 'direct' };
...
export interface ConnectorConfig {
   ...
   *
   * Only direct TCP is supported. `ConfigLoader.validateConfig` always
   * populates this field with `{ type: 'direct' }` when absent. The field is
   * declared optional at the TypeScript level only for backward compatibility
   * with call sites that construct partial `ConnectorConfig` literals in tests.
   */
...
export interface PeerInfo {
  ...
  /**
   * Effective per-peer transport. Only direct TCP is supported. `undefined`
   * when the peer inherits the connector-level default.
   */
  transport?: 'direct';
