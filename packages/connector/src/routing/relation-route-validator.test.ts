...
describe('deriveLocalPrefixes', () => {
  it('returns prefixes whose nextHop is the nodeId or local', () => {
    const routes = [
      { prefix: 'g.townhouse', nextHop: 'g.townhouse' },
      { prefix: 'g.local', nextHop: 'local' },
      { prefix: 'g.townhouse.town', nextHop: 'town' },
    ];
    expect(deriveLocalPrefixes(routes, 'g.townhouse')).toEqual(['g.townhouse', 'g.local']);
  });

  it('returns empty when nothing terminates locally', () => {
    const routes = [{ prefix: 'g.townhouse.town', nextHop: 'town' }];
    expect(deriveLocalPrefixes(routes, 'g.townhouse')).toEqual([]);
  });
});

describe('validateRelationRoute', () => {
  const self = ['g.townhouse'];

  it('accepts a child route under the connector address', () => {
    expect(validateRelationRoute('child', self, ['g.townhouse.town'])).toEqual({ ok: true });
  });
  ...
  it('rejects a child route equal to the connector address (not a strict descendant)', () => {
    expect(validateRelationRoute('child', self, ['g.townhouse']).ok).toBe(false);
  });

  it('rejects a parent route that sits under the connector subtree (child-shaped)', () => {
    const result = validateRelationRoute('parent', self, ['g.townhouse.upstream']);
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it('accepts any peer (lateral) route without subtree constraint', () => {
    ...
    expect(validateRelationRoute(undefined, self, ['g.townhouse.x'])).toEqual({ ok: true });
  });
  ...
  it('validates every route prefix, not just the first', () => {
    const result = validateRelationRoute('child', self, ['g.townhouse.town', 'g.other.mill']);
    expect(result.ok).toBe(false);
  });
});
...
describe('deriveDefaultChildRoute', () => {
  it('derives <self>.<peerId> for a child with no explicit route', () => {
    expect(deriveDefaultChildRoute('child', ['g.townhouse'], 'town')).toEqual({
  ...
  it('returns null for non-child relations', () => {
    expect(deriveDefaultChildRoute('peer', ['g.townhouse'], 'town')).toBeNull();
    expect(deriveDefaultChildRoute('parent', ['g.townhouse'], 'town')).toBeNull();
  });
