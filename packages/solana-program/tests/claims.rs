...
async fn setup_funded_channel(
    ...
    (channel_pda, vault_pda, token_mint, token_account_a, token_account_b)
}
...
async fn submit_claim(
    ...
    let claim_ix = build_claim_instruction(&claimer.pubkey(), channel_pda, nonce, transferred_amount);
...
async fn test_claim_on_closed_channel_succeeds() {
    ...
    assert_eq!(read_u64_at(&account.data, TRANSFERRED_AMOUNT_A_OFFSET), 3000);
...
async fn test_balance_proof_message_format() {
    ...
    assert_eq!(message.len(), 48, "Balance proof message must be exactly 48 bytes");
