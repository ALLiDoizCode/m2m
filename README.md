...
| `[[pay_channels]]`      | array       | see below | `{ peer_id, channel_id, chain_id, token_network, client_edge_url }`, or the Solana shape `{ peer_id, channel_account, client_edge_url }` — the channel this node **pays** that peer from. **Required for every peering a `[[routes]]` entry forwards to.** |
...
A `[[pay_channels]]` entry is the other direction, and the two are not interchangeable:
`[[peer_channels]]` is the channel a peering's claims are **judged** against, `[[client_channels]]`
is a channel this node **receives** claims on, and `[[pay_channels]]` is one it **pays** from — the
channel every PREPARE forwarded to that peer carries a covering claim on
([ADR 0042](docs/adr/0042-a-packet-carries-its-claim.md)). It is **required** of any peering a
`[[routes]]` entry forwards to (issue #1145): a connector covers every PREPARE it sends, there is no
uncovered path left for a forward to take, and a route to a peering with no row here is refused at
load naming both. A peering this node only _accepts_ on needs no row. `client_edge_url` is that
peer's own `POST /ilp` endpoint — where this node arrives
as an ordinary buyer, and where `POST /ilp/claim-state` answers where its claims on the channel
...
carries — claim exchange, flush, fees, the refusal taxonomy — are
