#!/usr/bin/env bash
# Issue a Let's Encrypt cert for the TOON faucet box.
# Adapted from infra/linode-relay/init-letsencrypt.sh (same idempotency logic).
set -euo pipefail
...
DC=(docker compose -f infra/linode-faucet/docker-compose.faucet.yml)

# ONE CERTIFICATE PER NAME (#830) — this box has exactly one public name, so
# there is no SAN-bundling temptation, but the mechanics below still follow
# the fleet's standing convention (every name in CERT_NAMES gets its own
# independent certbot request and its own lineage) so a second name added
# later needs no rewrite here.
CERT_NAMES=("faucet.${DOMAIN}")
...
# start at all if any `ssl_certificate` file named in the config is missing.
NEEDS_ISSUE=()
...
# Pass 2: issue each lineage on its own.
