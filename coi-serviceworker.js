 ...
 * Cross-Origin-Embedder-Policy: require-corp response headers CLIENT-SIDE, so a
 * static host that cannot set them (GitHub Pages) still becomes
 * `crossOriginIsolated` — the precondition for SharedArrayBuffer, which o1js's
 * prover uses to multi-thread. On first load the SW registers and reloads once;
 * from then on every response is patched.
 */
/* eslint-disable */
