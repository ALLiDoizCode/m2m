...
help:
	...
	@echo ""
		@echo ""
	@echo "App behind the connector (issue #221):"
	@echo "  make app-up               One-command up: connector + app + anvil + faucet"
	@echo "  make app-down             Tear down the app stack"
	@echo "  make app-logs             Follow the app docker compose logs"
...
# App behind the connector (issue #221) — the "hello-world" of deploying an app
# behind the connector locally. `make app-up` brings up a standalone
# connector (paid reverse proxy) + an oblivious relay (app) + anvil + faucet with
# one command (AC4). The relay image is env-overridable (`RELAY_IMAGE`) because the
