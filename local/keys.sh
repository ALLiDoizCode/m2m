...
if [[ -z "$TOPOLOGY" ]]; then
  ...
  echo "       topology: solo, two-hop, mixed-chain, onion" >&2
...
# `local/onion/*.toml` are committed with this host wherever the real `.onion`
# address goes, and this script substitutes the daemon's own address for it
# (see `render_onion_configs`). It is 56 characters of the base32 alphabet a
# v3 onion address is drawn from, and it ends in `.onion`, so the committed
...
# steps later.
ONION_PLACEHOLDER=placeholderplaceholderplaceholderplaceholderplaceholderb.onion
...
# The `.onion` address does not exist until the daemon has run. `anon`
...
render_onion_configs() {
  ...
  if [[ ! "$address" =~ ^[a-z2-7]{56}\.onion$ ]]; then
    echo "ERROR: anon-b has not produced a v3 onion address." >&2
  ...
  for entry in $NODES; do
    ...
    fi
    {
      ...
      echo "# The .onion host below was read out of anon-b's own"
