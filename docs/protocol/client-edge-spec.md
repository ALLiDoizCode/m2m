...
2. **Freshness** — the claim's nonce MUST strictly advance this connector's last-verified
   ...
   channel's watermark by a canonical form of the id: for `evm`, the `channelId`'s 32 bytes,
   normalised (lower-case hex, `0x` optional) before the watermark is written or read; for
   `solana`, the `channelAccount` as it arrives, base58 of an exact 32-byte decode having only
   one spelling already. Hex is case-insensitive and everything else about a claim already treats
