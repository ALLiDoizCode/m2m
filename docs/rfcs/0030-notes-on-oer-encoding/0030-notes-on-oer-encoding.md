...
>
> - **Upstream:** `interledger/rfcs` → `0030-notes-on-oer-encoding/0030-notes-on-oer-encoding.md`
> - **Pinned commit:** `1eb8d73b67a1d048f74ded508406a7e1ae1e00d5`
> - **Body SHA-256:** `d4a313794d92f5b121e5edb4f82b2f6ddd9770c29c6964c9d2d2cd7997c179ed`
> - **Licence:** CC BY-SA 4.0 — see [`docs/rfcs/README.md`](../README.md)

## TOON profile

**This connector's OER is a strict subset of this document, enforced harder than
this document requires.** Where RFC 0027 permits an implementation to accept
non-canonical encodings, this one refuses them.

- **Length determinants must be canonical.** `decode_var_uint` re-encodes what it
  just decoded and refuses anything not byte-identical: a non-minimal long form
  (`0x81 0x03` for `3`), the zero-length long form `0x80` aliasing `0x00`, and
  any determinant wider than eight bytes — which the previous accumulator
  silently truncated
  ([ADR 0023](../../adr/0023-oer-length-determinants-are-canonical.md);
  `crates/connector-domain/src/oer.rs`).
- **The tightening lives in the shared primitive, so the packet codec inherits
  it.** `Prepare`, `Fulfill` and `Reject` refuse a non-canonical `amount`,
  `data`, `destination`, `message` or `triggeredBy` length with no code of their
  own. ADR 0023 considered and rejected an envelope-only check, which would have
  left the packet codec as the one place a packet had more than one valid
  encoding.
- **Trailing bytes after an otherwise-complete decode are refused.** The property
  is stated as: every byte sequence either codec accepts re-encodes to exactly
  itself (`wire-vectors.md` §2). This is the vectored half of ADR 0023.
- **Two named errors rather than a lenient catch-all.**
  `PacketError::NonCanonicalLength` and `PacketError::LengthDeterminantOverflow`
  are distinct from `BufferUnderflow`, and the distinction is mirrored into
  `EnvelopeError` so it survives the envelope boundary.
- **OER encodes a second, non-ILP message family here.** The app request envelope
  — method, target, header sequence, body; status, header sequence, body — uses
  these same primitives rather than HTTP text, deliberately, so nothing parses
  HTTP at a paid boundary
  ([ADR 0018](../../adr/0018-a-payload-is-sealed-to-the-terminating-connector.md)).
- **The primitive set is narrower than this document's.** Only `VarUInt`,
...
`u64::MAX` round-tripping at the widest canonical determinant. Both properties
are property-tested rather than example-tested.

**One caveat on the record.** ADR 0023's Status line flags that its "safe for the
wire" argument reasons from a TypeScript file no longer in this repository. The
argument is historical; only the canonicality rule binds.
