...
>
> - **Upstream:** `interledger/rfcs` → `0001-interledger-architecture/0001-interledger-architecture.md`
> - **Pinned commit:** `1eb8d73b67a1d048f74ded508406a7e1ae1e00d5`
> - **Body SHA-256:** `1098390ebe58b1bdcddc16da22833e906263375476d9f8c8b6510ba5a005b60a`
> - **Licence:** CC BY-SA 4.0 — see [`docs/rfcs/README.md`](../README.md)

## TOON profile

**Read this RFC for the layering and the hop-by-hop model. Do not read it for
the layers above and below the connector — TOON has neither in the shape drawn
here.** The five-layer stack is the part of this document a TOON operator most
needs to hold loosely: two of the five are absent, and the middle one is where
this project lives.

- **There is no transport layer.** No STREAM, no chunking, no end-to-end secret
  negotiated above ILP. A packet's `data` is a gift wrap sealed to the
  **terminating connector's** identity key, carrying a 32-byte shared secret and
  an OER request envelope
  ([ADR 0018](../../adr/0018-a-payload-is-sealed-to-the-terminating-connector.md),
  restated as `packet-flow-spec.md` **PF-20**).
- **The application layer is ordinary HTTP the app never knows was paid for.**
  Where this RFC has an application protocol both endpoints speak, TOON has a
  payment-oblivious app behind a handler URL and a connector acting as a paid
  reverse proxy
  ([ADR 0020](../../adr/0020-a-price-is-flat-and-attaches-to-a-handler.md),
  narrowed by [ADR 0040](../../adr/0040-a-verified-payment-is-stated-to-the-app.md);
  `payment-spec.md` **PM-21**, **PM-22**).
- **The ledger layer is deleted rather than abstracted.**
  [ADR 0005](../../adr/0005-claims-are-truth-balances-are-a-projection.md) says
  it outright: there is no ledger abstraction. Settlement is in-process payment
  channels on EVM and Solana only
  ([ADR 0002](../../adr/0002-drop-mina-from-the-rust-connector.md)), with a
  channel derived from its participants
...
- **Conditional transfer is inverted.** Nothing is held in escrow pending a
  ...
  **PM-12**). Note that
  [ADR 0004](../../adr/0004-value-moves-on-fulfilment.md)'s title states the
  opposite and is **partly superseded by 0042** — cite 0042.
- **The sender–connector–receiver triad collapses to two roles.** There is no
  ...
  itself**, and what sits behind it is an **app**
  ...
  [`CONTEXT.md`](../../../CONTEXT.md) is the vocabulary and forbids "terminator",
  "BLS" and "agent runtime" for this reason.

**Faithful:** the link layer's bilateral peering shape — two carriages, BTP over
`wss://` and ILP-over-HTTP over `https://`
([ADR 0027](../../adr/0027-connectors-peer-over-btp-or-http-and-the-raw-tcp-peer-wire-is-deleted.md));
hop-by-hop forwarding with strictly decreasing expiry (`packet-flow-spec.md`
**PF-19**) and non-increasing amounts (**PF-13**); and payload opacity in
carriage, made structural rather than advisory
