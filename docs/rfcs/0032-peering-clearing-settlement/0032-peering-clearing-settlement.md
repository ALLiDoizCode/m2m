...
>
> - **Upstream:** `interledger/rfcs` → `0032-peering-clearing-settlement/0032-peering-clearing-settlement.md`
> - **Pinned commit:** `1eb8d73b67a1d048f74ded508406a7e1ae1e00d5`
> - **Body SHA-256:** `5eeba1e6ce3292d3403b69c877cda702eb0f73ceab0620c7e9f7caca1b70bfc6`
> - **Licence:** CC BY-SA 4.0 — see [`docs/rfcs/README.md`](../README.md)

## TOON profile

**This is the RFC whose model TOON replaces most completely. Read it to
understand what a peering is _for_; do not read it for how this connector keeps
score.** The three-phase peer / clear / settle cycle — accumulate an obligation,
net it, settle when it crosses a threshold — has no counterpart here. There is
nothing to net, because nothing accumulates.

- **Clearing is per packet.** Every PREPARE carries the claim that pays for it,
  and a connector covers every PREPARE it sends
  ([ADR 0042](../../adr/0042-a-packet-carries-its-claim.md); `payment-spec.md`
  **PM-12**, **PM-13**, **PM-15**). `CONTEXT.md` puts the consequence plainly:
...
- **There is no settlement threshold, trigger, netting cycle or interval.**
  Settlement is an explicit operator write against a channel, never a schedule;
  `flush_interval_ms` is parsed only to be refused by name
...
- **There is no credit limit.** `CONTEXT.md`'s retired _ceiling_ entry refuses
  "credit limit" and "debt limit" by name. What remains is the **cap**, which
  bounds **one packet** and never an accumulation
  ([ADR 0049](../../adr/0049-the-cap-bounds-one-packet-is-discovered-by-t04-and-is-set-from-outside.md);
  `packet-flow-spec.md` **PF-18**).
- **Terms are not negotiated.** A peering is written by one operator from a URL —
  `POST /peers { id, url, fee, max_packet_amount }` — with the counterparty's
  endpoint, carriage, identity and settlement facts read from its
  self-description
  ([ADR 0058](../../adr/0058-a-peering-is-established-from-a-url.md);
  `operator-spec.md` **OP-07**; `configuration-spec.md` **CF-16**). That identity
  is trust-on-first-use over TLS, pinned by nothing — worth knowing before you
  peer with a stranger.
- **No channel identifier is ever exchanged.** Both sides derive it from the two
  participants — `keccak256(p1, p2, epoch)` on EVM, a PDA on Solana
  ([ADR 0059](../../adr/0059-a-channel-is-derived-from-its-participants.md)).
- **The fee is flat, per packet, and attached to the peering** — not a spread, not
  a percentage, not per route
  ([ADR 0061](../../adr/0061-a-fee-attaches-to-a-peering-not-to-a-route.md);
  **PM-17**).

**Faithful:** the bilateral relation is still the unit — one relation, one fee,
one watermark set, however many carriage paths carry it
(`peer-carriage-spec.md` §2.5). On-chain settlement is rare and deliberate, the
opposite of a claim. And the payee's remedy for a watermark that stops advancing
is this RFC's own: stop forwarding to that peer (`peer-carriage-spec.md` §6.2).

**What this trade costs, stated where an operator will meet it.** Because the
claim rides with the PREPARE rather than after the fulfilment, a hop can take
your claim and decline to carry the packet. ADR 0042 does not hide this. The
bounds on it are the per-packet cap and the sender's own choice of packet size —
which is this RFC's neighbour, RFC 0018, giving the same advice for a different
reason.
