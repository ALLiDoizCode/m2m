...
>
> - **Upstream:** `interledger/rfcs` → `0019-glossary/0019-glossary.md`
> - **Pinned commit:** `1eb8d73b67a1d048f74ded508406a7e1ae1e00d5`
> - **Body SHA-256:** `0d7e8012eb697089d5b9ed66a3cc17283883e8773d0ee36db7378a3ba935bc65`
> - **Licence:** CC BY-SA 4.0 — see [`docs/rfcs/README.md`](../README.md)

## TOON profile

**[`CONTEXT.md`](../../../CONTEXT.md) is this project's vocabulary and it wins.**
Where a term below and a term there disagree, `CONTEXT.md` is what the code, the
records and the specs use — and the rule the ADR index states is to fix the
glossary, never the record. Read this RFC to understand what an Interledger
practitioner means; read `CONTEXT.md` to understand what a line of this
repository means.
...
- **"Receiver" is not a role.** The triad becomes **connector**, **app** and
  **handler**. "Terminator", "BLS", "Business Logic Server", "agent runtime",
  "gateway" and "backend" are forbidden names — by `CONTEXT.md` and `CLAUDE.md`
  rather than by a record, because no record ever used them.
- **"Exposure" is retired outright, with nothing replacing it.** No projection
  produces it; `ceiling` and `flush_interval_ms` are parsed only to be refused by
  name
  ([ADR 0033](../../adr/0033-the-exposure-machinery-is-retired-not-restated.md);
  `payment-spec.md` **PM-16**). **Ceiling** and **flush** go with it, and
  **liquidity bound** is refused as a framing: a **cap** bounds one packet, never
  an accumulation
  ([ADR 0049](../../adr/0049-the-cap-bounds-one-packet-is-discovered-by-t04-and-is-set-from-outside.md);
  `packet-flow-spec.md` **PF-18**).
- **"Fulfillment" is respelled and redefined.** British _fulfilment_, and it
  proves delivery; it does not move value
  ([ADR 0042](../../adr/0042-a-packet-carries-its-claim.md)). _Receipt_, _proof
  of payment_ and _preimage_ are all avoided.
- **"Peer" is narrower, and peer role is proved by a claim signature** — never by
  ...
  A peering is established from a URL
...
- **"Quote" and "spread" are refused.** A **fee** is not a spread, a commission
  ...
  reject carrying the accumulated sum
  ([ADR 0011](../../adr/0011-rejects-accumulate-fees-and-probes-discover-cost.md)).
- **"Minimum delivery" and "peer wire" are tombstones**, kept in `CONTEXT.md`
  only so the names are refused rather than reinvented
...
**Faithful:** _connector_, _packet_, _condition_, _prepare_ / _fulfil_ / _reject_,
_expiry_, _peering_ and _settlement_ keep their meanings here. _Condition_ is
kept and strengthened — a real execution condition is mandatory and there is no
zero-condition path anywhere (`packet-flow-spec.md` **PF-01**).
