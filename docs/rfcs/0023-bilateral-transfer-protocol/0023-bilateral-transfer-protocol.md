...
>
> - **Upstream:** `interledger/rfcs` → `0023-bilateral-transfer-protocol/0023-bilateral-transfer-protocol.md`
> - **Pinned commit:** `1eb8d73b67a1d048f74ded508406a7e1ae1e00d5`
> - **Body SHA-256:** `dd2b3657c5534df8d4b12ed4d91c4a2905f84b36578e183cbc2bd711d29e094c`
> - **Licence:** CC BY-SA 4.0 — see [`docs/rfcs/README.md`](../README.md)

## TOON profile

**This is one of the two carriages a peering rides — `wss://` picks it, `https://`
picks ILP-over-HTTP
([ADR 0027](../../adr/0027-connectors-peer-over-btp-or-http-and-the-raw-tcp-peer-wire-is-deleted.md)).
Read it for the frame model. The authentication half does not apply here at all.**

- **⚠ The frame grammar is not this RFC's ASN.1/OER grammar.** It is the deployed
  `@toon-protocol/client` dialect: fixed-width `type(u8) requestId(u32)
pdCount(u8) … ilpLen(u32) ilpPacket`, with the ILP packet riding **beside** the
  protocol-data list rather than inside it as an `ilp` sub-protocol entry
  (`client-edge-spec.md` §1.9;
  [ADR 0026](../../adr/0026-client-btp-rides-the-client-edge-peers-stay-on-the-peer-wire.md)
  argues why the deployed dialect won over the RFC's grammar). That dialect's
  vectors are in-crate Rust unit tests a non-Rust consumer cannot replay, so it
  is **not** covered by ADR 0021's cross-repo contract.
- **The `auth` frame authenticates nothing and admits nothing.** There are no
  `auth_token` or `auth_username` sub-protocols; the single `auth` entry carries
  JSON `{peerId, secret}` whose contents are not verified, and a session
  presenting `secret: ""` — or no `auth` frame at all — is admitted as a client
  (`client-edge-spec.md` §1.9 step 1). Nothing about the handshake is trusted.
- **The peer half of `auth` is deleted outright.** A peering carries no
  credential of any kind
  ([ADR 0060](../../adr/0060-a-claim-proves-a-peering-and-the-shared-secret-is-deleted.md)).
  Nothing replaced it: role is decided per frame by a `[[peer_channels]]` binding
  plus a claim whose signature verifies against that row's configured
  counterparty key. A stale peer `auth` entry is ignored, never refused.
  `[[peers]].credential` is a boot-time tombstone.
- **Role is a property of the frame, not the session.** A session never "becomes"
  a peer and stays one, which inverts this RFC's session-scoped authentication
  (`peer-carriage-spec.md` §1.5). The peer wire's `schema_version` is 4.
- **TRANSFER is repurposed.** On the client edge it is acknowledged with an empty
  RESPONSE and its settlement accounting is out of scope. On the peer carriage
  TRANSFER **is** the flush: `amount` is the claim's new cumulative, the claim
  rides `payment-channel-claim`, and there is no `ilpPacket`
  (`peer-carriage-spec.md` §3).
- **ERROR is narrowed to undecodable frames.** A rejected _claim_ must not be an
  ERROR frame and must not change the packet's outcome — one RESPONSE carries two
  independent answers, the `ilpPacket` and the `claim-ack`
  (`peer-carriage-spec.md` §6.2). This is why a peering can carry traffic for
  free without the packet ever failing, and why `local/`'s peered topologies read
  the payee's claim journal rather than trusting a fulfilment.
- **Five sub-protocol entries this RFC does not define**, and inventing a sixth is
  forbidden: `payment-channel-claim`, `claim-ack`, `toon-accumulated-cost`,
  `payment-required`, `payout-claim` (`peer-carriage-spec.md` §3).

**Faithful:** the type numbers (RESPONSE 1, ERROR 2, MESSAGE 6, TRANSFER 7); one
frame per binary websocket message; the responder-answers-every-request property,
which the claim-ack timeout bound leans on; the symmetric grammar; and the
requestId uniqueness rule, with a session-scoped allocator. This RFC's own NAT
motivation is why peers ride BTP at all (ADR 0027).

**One stale citation to avoid.** `client-edge-spec.md` §1.9's "Peer sessions"
paragraph still says a session is a peer session if it presented a credential
configured in `[[peers]]`. That is pre-ADR-0060. Cite
`peer-carriage-spec.md` §1.4 for peer role.
