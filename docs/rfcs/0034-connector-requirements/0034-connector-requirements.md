...
>
> - **Upstream:** `interledger/rfcs` → `0034-connector-requirements/0034-connector-requirements.md`
> - **Pinned commit:** `1eb8d73b67a1d048f74ded508406a7e1ae1e00d5`
> - **Body SHA-256:** `74f68f6e3a9db2f68313224e099d6fa6e5b637a466ef112ff402941131935f64`
...
- **No route discovery, no advertisement, no CCP.** This connector never learns,
  ...
  ([ADR 0006](../../adr/0006-the-connector-is-mechanism-not-policy.md), restored
  in full by
  [ADR 0043](../../adr/0043-purchasable-peering-is-removed.md)); the kind:10032
  announce was removed
  ([ADR 0046](../../adr/0046-the-kind-10032-announce-is-removed-a-connector-needs-no-relay.md)),
  and `operator-spec.md` **OP-06** makes it a MUST NOT "on any schedule, ever."
- **It answers instead.** A free, unauthenticated `GET` on the node's own URL
  returns its self-description
  ...
  `self-description-spec.md` **ND-01**–**ND-03**), and that document must **not**
  carry per-peer facts, fees or caps (**ND-09**).
- **The routing table is static, layered, and precedence is length then kind** —
  terminated, forwarded, runtime, leased, with length dominating kind
  ([ADR 0048](../../adr/0048-routing-precedence-is-length-then-rank-and-a-lease-cannot-capture-a-termination.md);
  `packet-flow-spec.md` **PF-03**–**PF-08**). A runtime row may never shadow a
  config-file key
...
- **A fee is flat, per packet, and attaches to a peering rather than a route**
  ...
  amended by
  [ADR 0061](../../adr/0061-a-fee-attaches-to-a-peering-not-to-a-route.md) — a
  route's `fee` is now a refuse-to-start tombstone). A **price** is flat and
  attaches to a handler
  ([ADR 0020](../../adr/0020-a-price-is-flat-and-attaches-to-a-handler.md);
  `payment-spec.md` **PM-17**, **PM-18**).
- **Balance and liquidity management is replaced by a per-packet cap and a
  covering claim.** Nothing accumulates between packets
  ([ADR 0042](../../adr/0042-a-packet-carries-its-claim.md)), and `T04` is
  repurposed from this RFC's _Insufficient Liquidity_ to "this packet exceeds the
  cap, and here is the cap"
  ([ADR 0049](../../adr/0049-the-cap-bounds-one-packet-is-discovered-by-t04-and-is-set-from-outside.md);
  `packet-flow-spec.md` **PF-18**). A connector must not raise its own cap
  (`configuration-spec.md` **CF-20**).
- **An app's `404` is not a reject.** It rides home on a FULFILL, because the app
  answered and the answer is what was paid for. Only unreachability (`T01`) or a
  refused envelope target (`F00`) produces a reject (`packet-flow-spec.md`
  **PF-23**).

**Faithful:** longest-prefix routing, made label-aware so `g.example` cannot
match `g.exampleX` (**PF-03**); expiry handling — an already-expired packet is
refused before routing (**PF-02**) and expiry decreases on every forward by
enough for the fulfilment to make it home (**PF-19**); RFC 0027's error-code set
and its F/R/T class semantics
([ADR 0051](../../adr/0051-a-reject-code-binds-where-a-sender-must-act-differently.md));
and accepting payment from an unregistered buyer whose channel is resolved from
chain, which this RFC's spirit invites and
[ADR 0052](../../adr/0052-permissionless-payment-is-guaranteed-and-a-claim-is-what-authorises.md)
makes a MUST (`payment-spec.md` **PM-05**).
