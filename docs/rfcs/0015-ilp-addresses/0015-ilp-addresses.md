...
>
> - **Upstream:** `interledger/rfcs` → `0015-ilp-addresses/0015-ilp-addresses.md`
> - **Pinned commit:** `1eb8d73b67a1d048f74ded508406a7e1ae1e00d5`
> - **Body SHA-256:** `33950d7e0a9195d044c69a8c489c33a56d99f1e90ea7527666bfb2e0ccd0c580`
> - **Licence:** CC BY-SA 4.0 — see [`docs/rfcs/README.md`](../README.md)

## TOON profile

**Read this for the address grammar and for longest-prefix matching. The
allocation schemes it defines have no behaviour in this connector at all.**

- **An address is self-asserted: a claim, not a grant.** Nothing allocates one,
  no registry records one, and no connector is given one by another —
  [`CONTEXT.md`](../../../CONTEXT.md) puts it as _reachability is the only
  registry_, and refuses the phrases "allocated address", "assigned address" and
  "address space" as though it were owned. Choosing a name beneath a peer's
  address is a courtesy that keeps their table small, never a delegation that
  binds anyone.
- **The allocation scheme is not validated, or even read.** `is_valid_ilp_address`
  checks non-empty, ≤ 1023 characters, and that every dot-separated label is
  valid. There is no enumeration of `g` / `private` / `example` / `peer` /
  `self` / `test[1-3]` / `local`, and a bare `"g"` passes
  (`crates/connector-domain/src/address.rs`, duplicated in
  `crates/connector-config/src/route.rs`).
- **`peer.`, `self.`, `private.`, `example.` and `test.` have no special
  handling anywhere.** `peer.`'s link-local semantics, `self.`'s loopback and
  `private.`'s unroutability are simply absent: there is no scheme-derived
  routing behaviour, only the route table. Every address in `infra/`, `local/`,
  `crates/` and `docs/` is under `g.`.
- **⚠ The label character set omits `~`.** Only ASCII alphanumerics plus `-` and
  `_` are accepted, and the doc comment claims this matches RFC 0015. It is in
  fact stricter, inherited from the TypeScript prototype. A conforming peer using
  `~` in a label would be refused.
- **Matching is longest-prefix and then route rank** — App, Peer, RuntimePeer,
  Leased — with length dominating rank, an ordering this RFC has no notion of
  ([ADR 0048](../../adr/0048-routing-precedence-is-length-then-rank-and-a-lease-cannot-capture-a-termination.md);
  `packet-flow-spec.md` **PF-03**–**PF-08**). Matching is label-aware, so
  `g.example` cannot match `g.exampleX`. One exception — a lease must not
  out-specify a terminated route's subtree (**PF-06**) — is **not yet built**
  (issue #1078).
- **A destination can resolve outside the hierarchy entirely.** A live client
  session is an exact-address hash lookup, not part of the prefix ordering, and
  receives only what the route table could not place. An address matching both a
  session and a terminated route is a `T00` configuration error rather than a
  precedence question
  ([ADR 0032](../../adr/0032-a-client-destination-is-never-a-route-termination.md);
  **PF-09**–**PF-11**).
- **An address does not name an endpoint by itself.** The envelope's target is
  confined beneath the matched route's handler path: `""` and `"/"` mean the
  handler's own path, and any other absolute path, `.`/`..` segment, scheme,
  authority, backslash or percent-encoded spelling of those is `F00` before the
  app is touched
  ([ADR 0025](../../adr/0025-an-envelope-target-is-confined-beneath-the-handler-path.md);
  **PF-21**).

**Faithful:** dot-separated labels as the atom of matching, which ADR 0048 cites
this RFC for; the 1023-character maximum; no leading, trailing or consecutive
dots and no empty label; hierarchical longest-prefix routing as the base rule;
and an empty address permitted only as a REJECT's `triggeredBy`.

**Do not cite [ADR 0013](../../adr/0013-cut-over-through-a-parallel-address-space.md)
as address law.** Its parallel address space was a temporary `g.`-space prefix
for running the Rust fleet beside the TypeScript one, not a second allocation
scheme; it is filed under fleet operations, its comparison half is superseded by
ADR 0017, and it is otherwise spent.
