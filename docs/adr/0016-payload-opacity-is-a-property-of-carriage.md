...
> asked not to. The second half — that the Rust connector must implement client edge version 1 as
> written, with conformance defined as wire-identical behaviour and the TypeScript implementation as
> the behavioural reference — is superseded by ADR 0017. There is no conformance target; the
> TypeScript connector is a prototype. Read the rest of this document for why the envelope must be
> read, not for what the Rust connector must implement.
