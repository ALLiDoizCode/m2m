...
- `fee` — unchanged from ADR 0010: what **this hop retains** for carriage, realized on the wire as
  the difference between the amount received and the amount forwarded (`peer-wire-spec.md` §4).
...
collects `price` from the client, forwards `price - fee` (`peer-wire-spec.md` §4, unchanged,
