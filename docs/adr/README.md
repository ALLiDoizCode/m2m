...
| 0042   | a covering claim on forwarded arrivals                                  | **Built** (#1142), defaulting to observe per peering, so no deployed box is bound yet. The cap and the send half (`[[pay_channels]]`) are built too. `ClaimEnforcement::Observe` — once listed here — is **deleted** (#1077). See its Status line. |
