...
- **Lost.** The body shape changes: today
  ...
  and it does not read the body: `infra/devnet-manage.sh:416` — `probe "https://proxy.$DOMAIN/health" "proxy/connector"`
...
2. Sweep for `/health` body consumers — **done, in §2.1**. The only consumer is
   `infra/devnet-manage.sh:416`, and it checks reachability, not body.
